...
import com.example.RadioAPI.config.APIconfig;
...
public class RadioAPIservice {
    ...
    public List<RadioAPImodel> listRadioAPImodel() {
        ...
        try {
            ...
            List<RadioAPImodel> radioStations = extractRadioStations(response.getBody());

            // Ordena a lista por votos (decrescente)
            radioStations.sort(Comparator.comparingInt(RadioAPImodel::getVotes).reversed());
    ...
    private List<RadioAPImodel> extractRadioStations(List<Map<String, Object>> stationsData) {
        List<RadioAPImodel> radioStations = new ArrayList<>();

        if (stationsData != null) {
            for (Map<String, Object> stationMap : stationsData) {
                RadioAPImodel station = new RadioAPImodel();
